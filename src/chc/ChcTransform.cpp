...
namespace {
struct PointerRef {
    LocalMemoryObject const * object;
    MyVariable index;
};

using LocalMemoryState = std::map<LocalMemoryObject const *, MyVariable>;

MyVariable make_unsigned_constant(std::uint64_t value, BitvectorType type) {
    auto string_value = std::to_string(value);
    return MyVariable::integerConstant(string_value, string_value, type);
}

bool is_zero(MyVariable const & value) {
    return value.isConstant && value.isIntegerConstant && value.bitvectorValue == "0";
}
} // namespace
...
std::uint64_t scalar_element_count(Type const * type) {
    if (type->isIntegerTy()) { return 1; }
    if (auto const * array = dyn_cast<ArrayType>(type)) {
        auto const element_count = scalar_element_count(array->getElementType());
        return array->getNumElements() * element_count;
    }
    throw UnsupportedFeature("Only integer elements are supported in local stack arrays.");
}

std::vector<LocalMemoryObject> collect_local_memory_objects(Function const & function) {
    std::vector<LocalMemoryObject> result;
    auto const & data_layout = function.getParent()->getDataLayout();
    unsigned id = 0;

    for (Instruction const & instruction : instructions(function)) {
        auto const * allocation = dyn_cast<AllocaInst>(&instruction);
        if (!allocation) { continue; }
        Type const * allocated_type = allocation->getAllocatedType();
        Type const * element_type = allocated_type;
        if (auto const * array = dyn_cast<ArrayType>(allocated_type)) {
            // One source-level array dimension is enough for the first
            // milestone.  A variable-size alloca of scalar elements covers
            // flattened VLAs generated by Clang.
            if (array->getElementType()->isArrayTy()) {
                throw UnsupportedFeature("Nested local arrays are not supported yet.");
            }
            element_type = array->getElementType();
        }
        if (!element_type->isIntegerTy()) {
            throw UnsupportedFeature("Only integer local arrays are supported.");
        }

        auto const * index_type = data_layout.getIndexType(allocation->getType());
        if (!index_type->isIntegerTy()) {
            throw UnsupportedFeature("Unsupported pointer index type for local array.");
        }
        result.push_back(LocalMemoryObject{
            .allocation = allocation,
            .id = id++,
            .index_type = get_type(index_type),
            .element_type = get_type(element_type),
            .allocation_count = allocation->getArraySize(),
            .static_element_count = scalar_element_count(allocated_type),
        });
    }
    return result;
}
...
MyFunctionInfo load_my_function_info(Function const & F) {
    ...
    function_info.local_memory_objects = collect_local_memory_objects(F);
...
MyVariable memory_state_variable(MyFunctionInfo const & function_info, MyBasicBlock const & block,
                                 LocalMemoryObject const & object, bool entry) {
    return MyVariable::variable("hornix_mem_" + function_info.name + "_" + std::to_string(object.id) +
                                    "_bb" + std::to_string(block.id) + (entry ? "_in" : "_out"),
                                object.type());
}

MyVariable memory_extent_variable(MyFunctionInfo const & function_info, MyBasicBlock const & block,
                                  LocalMemoryObject const & object, bool entry) {
    return MyVariable::variable("hornix_mem_extent_" + function_info.name + "_" + std::to_string(object.id) +
                                    "_bb" + std::to_string(block.id) + (entry ? "_in" : "_out"),
                                object.index_type);
}

LocalMemoryObject const * find_local_memory_object(MyFunctionInfo const & function_info, AllocaInst const * allocation) {
    for (auto const & object : function_info.local_memory_objects) {
        if (object.allocation == allocation) { return &object; }
    }
    return nullptr;
}
...
std::vector<Implication> Context::transform_basic_blocks(MyFunctionInfo & function_info) {
    ...
    for (auto const & [id, BB] : function_info.basic_blocks) {
        ...
        for (auto & succ : BB.successors) {
            ...
            // Local array values are explicit state threaded across CFG edges.
            // They are not SSA registers and therefore cannot be recovered by
            // the ordinary liveness/PHI machinery.
            for (auto const & object : function_info.local_memory_objects) {
                implication.constraints.push_back(std::make_unique<Equality>(
                    memory_state_variable(function_info, successor, object, true),
                    memory_state_variable(function_info, BB, object, false)));
                implication.constraints.push_back(std::make_unique<Equality>(
                    memory_extent_variable(function_info, successor, object, true),
                    memory_extent_variable(function_info, BB, object, false)));
            }
...
Implication::Constraints Context::transform_function_call(Instruction const * I, MyFunctionInfo & function_info) {
    ...
    auto has_prefix = [&](char const * prefix) { return function_name.rfind(prefix, 0) == 0; };
    bool const stack_intrinsic = has_prefix("llvm.stacksave") ||
                                 has_prefix("llvm.stackrestore") ||
                                 has_prefix("llvm.lifetime.");
    if (!stack_intrinsic) {
        for (Value const * argument : call_inst->args()) {
            if (argument->getType()->isPointerTy()) {
                throw UnsupportedFeature("Pointer arguments are not supported in the local-array fragment.");
            }
        }
    }
...
Implication::Constraints Context::transform_instructions(MyBasicBlock const & BB, MyFunctionInfo & function_info) {
    ...
    LocalMemoryState memory_state;
    LocalMemoryState memory_extent;
    std::map<Value const *, PointerRef> pointer_refs;
    unsigned temporary_index = 0;
    for (auto const & object : function_info.local_memory_objects) {
        memory_state.emplace(&object, memory_state_variable(function_info, BB, object, true));
        memory_extent.emplace(&object, memory_extent_variable(function_info, BB, object, true));
    }

    auto temporary_scalar = [&](BitvectorType type, std::string_view purpose) {
        return MyVariable::variable("hornix_" + std::string(purpose) + "_bb" + std::to_string(BB.id) + "_" +
                                        std::to_string(temporary_index++),
                                    type);
    };
    auto temporary_array = [&](LocalMemoryObject const & object) {
        return MyVariable::variable("hornix_mem_tmp_bb" + std::to_string(BB.id) + "_" +
                                        std::to_string(temporary_index++),
                                    object.type());
    };

    auto pointer_ref = [&](Value const * pointer) -> PointerRef {
        if (auto const * allocation = dyn_cast<AllocaInst>(pointer)) {
            auto const * object = find_local_memory_object(function_info, allocation);
            if (!object) { throw UnsupportedFeature("Unsupported local allocation."); }
            auto [it, inserted] = pointer_refs.emplace(pointer, PointerRef{object, make_unsigned_constant(0, object->index_type)});
            return it->second;
        }
        auto const it = pointer_refs.find(pointer);
        if (it == pointer_refs.end()) {
            throw UnsupportedFeature("Loads and stores require a non-escaping local array pointer.");
        }
        return it->second;
    };

    auto add_gep_offset = [&](MyVariable base, MyVariable offset, BitvectorType type) {
        if (is_zero(offset)) { return base; }
        if (is_zero(base)) { return offset; }
        auto sum = temporary_scalar(type, "gep_index");
        result.push_back(std::make_unique<BinaryConstraint>(sum, base, "+", offset));
        return sum;
    };
    ...
    for (Instruction const & I : BB.BB_link->instructionsWithoutDebug()) {
        switch (I.getOpcode()) {
            ...
            case Instruction::Alloca:
                if (auto const * object = find_local_memory_object(function_info, cast<AllocaInst>(&I))) {
                    auto extent = convert_operand_to_myvar(object->allocation_count);
                    if (extent.type.size() != object->index_type.size()) {
                        if (auto const * constant = dyn_cast<ConstantInt>(object->allocation_count)) {
                            extent = make_unsigned_constant(constant->getZExtValue(), object->index_type);
                        } else {
                            throw UnsupportedFeature("Local-array allocation size type is not supported.");
                        }
                    }
                    if (object->static_element_count != 1) {
                        auto scaled = temporary_scalar(object->index_type, "allocation_extent");
                        result.push_back(std::make_unique<BinaryConstraint>(
                            scaled, extent, "*", make_unsigned_constant(object->static_element_count, object->index_type)));
                        extent = std::move(scaled);
                    }
                    memory_extent.at(object) = std::move(extent);
                } else {
                    throw UnsupportedFeature("Unsupported local allocation.");
                }
                break;
            case Instruction::GetElementPtr: {
                auto const * gep = cast<GetElementPtrInst>(&I);
                auto ref = pointer_ref(gep->getPointerOperand());
                Type const * current_type = gep->getSourceElementType();
                MyVariable address = ref.index;
                unsigned position = 0;
                for (Value const * raw_index : gep->indices()) {
                    if (!raw_index->getType()->isIntegerTy() || get_type(raw_index->getType()).size() != ref.object->index_type.size()) {
                        throw UnsupportedFeature("Local-array GEP index type is not supported.");
                    }
                    std::uint64_t scale = 1;
                    if (position == 0) {
                        scale = scalar_element_count(current_type);
                    } else {
                        auto const * array = dyn_cast<ArrayType>(current_type);
                        if (!array) { throw UnsupportedFeature("Unsupported local-array GEP shape."); }
                        current_type = array->getElementType();
                        scale = scalar_element_count(current_type);
                    }
                    MyVariable offset = convert_operand_to_myvar(raw_index);
                    if (scale != 1) {
                        auto scaled = temporary_scalar(ref.object->index_type, "gep_scale");
                        result.push_back(std::make_unique<BinaryConstraint>(
                            scaled, offset, "*", make_unsigned_constant(scale, ref.object->index_type)));
                        offset = std::move(scaled);
                    }
                    address = add_gep_offset(std::move(address), std::move(offset), ref.object->index_type);
                    ++position;
                }
                if (!current_type->isIntegerTy()) {
                    throw UnsupportedFeature("Local-array GEP must address an integer element.");
                }
                pointer_refs.emplace(&I, PointerRef{ref.object, std::move(address)});
                break;
            }
            ...
            case Instruction::Load: {
                ...
                auto const ref = pointer_ref(I.getOperand(0));
                if (get_type(I.getType()).size() != ref.object->element_type.size()) {
                    throw UnsupportedFeature("Local-array load has an incompatible element type.");
                }
                result.push_back(std::make_unique<ComparisonConstraint>(ref.index, "uge", make_unsigned_constant(0, ref.object->index_type)));
                result.push_back(std::make_unique<ComparisonConstraint>(ref.index, "ult", memory_extent.at(ref.object)));
                result.push_back(std::make_unique<ArraySelectConstraint>(
                    convert_name_to_myvar(&I), memory_state.at(ref.object), ref.index));
                break;
            }
            case Instruction::Store: {
                ...
                auto const ref = pointer_ref(I.getOperand(1));
                auto value = convert_operand_to_myvar(I.getOperand(0));
                if (!value.type.isScalar() || value.type.size() != ref.object->element_type.size()) {
                    throw UnsupportedFeature("Local-array store has an incompatible element type.");
                }
                result.push_back(std::make_unique<ComparisonConstraint>(ref.index, "uge", make_unsigned_constant(0, ref.object->index_type)));
                result.push_back(std::make_unique<ComparisonConstraint>(ref.index, "ult", memory_extent.at(ref.object)));
                auto next_memory = temporary_array(*ref.object);
                result.push_back(std::make_unique<ArrayStoreConstraint>(
                    next_memory, memory_state.at(ref.object), ref.index, value));
                memory_state.at(ref.object) = std::move(next_memory);
                break;
            }
    ...
    for (auto const & object : function_info.local_memory_objects) {
        result.push_back(std::make_unique<Equality>(
            memory_state_variable(function_info, BB, object, false), memory_state.at(&object)));
        result.push_back(std::make_unique<Equality>(
            memory_extent_variable(function_info, BB, object, false), memory_extent.at(&object)));
    }
    return result;
}
...
MyPredicate Context::create_basic_block_predicate(MyBasicBlock const & BB, BasicBlockPredicateType const type,
    ...
    for (auto const & arg : function_info.llvm_function.args()) {
        if (arg.getType()->isPointerTy()) {
            if (!arg.use_empty()) {
                throw UnsupportedFeature("Pointer function arguments are not supported in the local-array fragment.");
            }
            continue;
        }
    ...
    for (auto const & object : function_info.local_memory_objects) {
        vars.push_back(memory_state_variable(function_info, BB, object, type == BasicBlockPredicateType::ENTRY));
        vars.push_back(memory_extent_variable(function_info, BB, object, type == BasicBlockPredicateType::ENTRY));
    }
